...
export const InfrastructurePage: React.FC<InfrastructurePageProps> = ({ onNavigate }) => {
  return (
    <div className="bg-white text-[#102A3A]">
            ...
            {INFRASTRUCTURE_MODULES.map((module, index) => {
              ...
              return (
                <div
                  key={module.id}
                  id={module.id}
                  className={`grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center p-6 sm:p-8 bg-[#073B57] text-white rounded-sm border border-[#0F5A82] hover:border-[#D9043E] transition-all`}
                >
                  <div className={`lg:col-span-6 space-y-4 ${isEven ? 'lg:order-2' : 'lg:order-1'}`}>
                    <div className="flex items-center gap-3">
                      <span className="text-sm font-mono font-bold text-white px-2 py-0.5 bg-[#D9043E] rounded">
                        {module.number}
                      </span>
                      <span className="text-xs font-bold text-[#D8EFFA] uppercase tracking-wider">
                        {module.tagline}
                      </span>
                    </div>
                    ...
                    <p className="text-sm text-[#D8EFFA] leading-relaxed">
                      {module.description}
                    </p>
                    ...
                    <div className="pt-2">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {module.features.map((feat, i) => (
                          <div key={i} className="flex items-start gap-2 text-xs text-[#D8EFFA]">
                  ...
                  <div className={`lg:col-span-6 ${isEven ? 'lg:order-1' : 'lg:order-2'}`}>
                    <SafeImage
                      ...
                      alt={module.title}
                      aspectRatio="video"
                      containerClassName="rounded-sm shadow-md border border-[#0F5A82]"
      ...
      <section className="py-16 bg-[#EAF6FC] border-t border-[#D8EFFA]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-xl mx-auto mb-10">
            <h3 className="text-xl font-bold uppercase tracking-tight text-[#102A3A]">
              CIVIL PROJECT GOVERNANCE & SAFETY
            </h3>
            <p className="text-xs text-[#3B586F] mt-1">Strict adherence to structural norms and statutory clearances.</p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="p-5 bg-white border border-[#D8EFFA] rounded-sm">
              <h4 className="text-sm font-bold text-[#102A3A]">Machinery Fleet</h4>
              <p className="text-xs text-[#3B586F] mt-1">Graders, asphalt compactors, excavators, and utility trenching equipment.</p>
            </div>
            <div className="p-5 bg-white border border-[#D8EFFA] rounded-sm">
              <h4 className="text-sm font-bold text-[#102A3A]">Utility Coordination</h4>
              <p className="text-xs text-[#3B586F] mt-1">Liaison and zero-disruption planning with municipal and power authorities.</p>
            </div>
            <div className="p-5 bg-white border border-[#D8EFFA] rounded-sm">
              <h4 className="text-sm font-bold text-[#102A3A]">Material Quality</h4>
              <p className="text-xs text-[#3B586F] mt-1">Rigorous batch testing for concrete, aggregates, and bituminous mixes.</p>
            </div>
            <div className="p-5 bg-white border border-[#D8EFFA] rounded-sm">
              <h4 className="text-sm font-bold text-[#102A3A]">Milestone Delivery</h4>
              <p className="text-xs text-[#3B586F] mt-1">Structured project timelines with on-site engineering supervision.</p>
            </div>
          </div>
        </div>
      </section>
